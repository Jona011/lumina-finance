...
const processingSteps = [
  'Reading sheets...', 'Identifying financial columns...', 'Normalizing dates and amounts...',
  'Detecting categories and transaction types...', 'Building financial model...', 'Generating insights and forecasts...',
];
...
export default function UploadPage() {
  ...
  const processFile = useCallback(async (f: File) => {
    if (!canUploadMore()) { setShowUpgrade(true); return; }
    setFile(f); setIsProcessing(true); setGlobalProcessing(true); setCurrentStep(0);
    try {
      setCurrentStep(0);
      const parsed = await parseSpreadsheetFile(f);
      setCurrentStep(1); await new Promise(r => setTimeout(r, 400));
      setCurrentStep(2); await new Promise(r => setTimeout(r, 400));
      setCurrentStep(3);
      ...
      if (user) {
        const { error } = await supabase.from('spreadsheets').insert({
          user_id: user.id, file_name: f.name, raw_data: parsed.rows as any, processed_data: processed as any,
        });
        if (error) console.error('Save error:', error);
      }
      await new Promise(r => setTimeout(r, 400));
      setCurrentStep(5); await new Promise(r => setTimeout(r, 600));
      setRealData(processed); setRawData(parsed.rows as any); setFileName(f.name); setHasData(true);
      incrementUploads(); setIsProcessing(false); setGlobalProcessing(false);
      toast.success('Spreadsheet processed successfully!');
      setTimeout(() => navigate('/dashboard'), 600);
    } catch (err) {
      console.error('Parse error:', err);
      toast.error('Failed to parse spreadsheet.');
      setIsProcessing(false); setGlobalProcessing(false);
    }
        ...
        <AnimatePresence mode="wait">
          {!isProcessing ? (
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              {!canUpload ? (
                <div className="glass-card p-12 text-center border-2 border-warning/30">
                  <Lock className="w-12 h-12 mx-auto mb-4 text-warning" />
                  <p className="text-lg font-medium mb-2">Upload limit reached</p>
                  <p className="text-sm text-muted-foreground mb-6">Upgrade to add more spreadsheets.</p>
                  <button onClick={() => setShowUpgrade(true)}
                    className="px-6 py-3 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover:brightness-110 transition-all shadow-lg shadow-primary/20">
                    Upgrade to Pro
                  </button>
                </div>
              ) : (
                <>
                  <div onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={handleDrop}
                    ...
                    <div className="flex items-center justify-center gap-3 text-xs text-muted-foreground">
                      <span className="px-3 py-1 rounded-full bg-muted">.xlsx</span>
                      <span className="px-3 py-1 rounded-full bg-muted">.csv</span>
                      <span className="px-3 py-1 rounded-full bg-muted">.xls</span>
                    </div>
                    <input id="file-input" type="file" accept=".xlsx,.csv,.xls" className="hidden" onChange={handleFileSelect} />
                  </div>
                  <div className="flex items-center justify-center gap-2 mt-4 text-xs text-muted-foreground">
                    <CheckCircle className="w-3.5 h-3.5 text-success" /> Your files are encrypted and processed securely
                  </div>
                </>
              )}
            </motion.div>
          ) : (
            <motion.div key="processing" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-8">
              ...
              <div className="space-y-3">
                {processingSteps.map((step, i) => (
                  <motion.div key={step} initial={{ opacity: 0, x: -10 }} animate={{ opacity: i <= currentStep ? 1 : 0.3, x: 0 }} transition={{ delay: i * 0.1 }}
                    className="flex items-center gap-3 text-sm">
                    {i < currentStep ? <CheckCircle className="w-4 h-4 text-success shrink-0" /> :
                     i === currentStep ? <div className="w-4 h-4 rounded-full border-2 border-primary border-t-transparent animate-spin shrink-0" /> :
              ...
              <div className="mt-6 h-2 rounded-full bg-muted overflow-hidden">
                <motion.div className="h-full rounded-full bg-primary" initial={{ width: '0%' }}
                  animate={{ width: `${((currentStep + 1) / processingSteps.length) * 100}%` }} transition={{ duration: 0.5 }} />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
