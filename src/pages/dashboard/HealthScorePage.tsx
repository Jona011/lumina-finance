  ...
  return (
    <div className="relative w-48 h-48 mx-auto">
      <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
        <circle cx="50" cy="50" r="45" fill="none" stroke="hsl(252,20%,90%)" strokeWidth="6" />
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-[1000px]">
      ...
      <div className="glass-card p-8 text-center">
        ...
        <p className="mt-4 text-success font-semibold">Good Standing</p>
        ...
        <div className="glass-card p-5">
          ...
          <div className="space-y-4">
            {healthScore.factors.map((f, i) => {
              ...
              const color = f.score >= 70 ? 'bg-success' : f.score >= 40 ? 'bg-warning' : 'bg-destructive';
              return (
                <motion.div key={f.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium">{f.name}</span>
                      <Icon className={`w-3.5 h-3.5 ${f.trend === 'up' ? 'text-success' : f.trend === 'down' ? 'text-destructive' : 'text-muted-foreground'}`} />
                    </div>
      ...
      <PremiumGate featureLabel="Improvement recommendations on Pro">
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="glass-card p-5 border-l-4 border-l-success">
            <h4 className="text-sm font-semibold mb-2 flex items-center gap-2"><ArrowUpRight className="w-4 h-4 text-success" /> What improves your score</h4>
